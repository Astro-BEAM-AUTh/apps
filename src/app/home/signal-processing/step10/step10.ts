...
@Component({
  ...
  styleUrl: './step10.scss'
})

export class Step10 {
  // ---- Parameters (consistent with earlier steps) ----
  numSamples = 500;
  centerFreq = 1420;
  bandwidth = 4;
  ...
  private generate(): void {
    // OFF: background + noise (time-average)
    ...
    // ON: background + spectral + noise (time-average)
    const onRows: number[][] = [];
    for (let s = 0; s < this.numSegments; s++) {
      const noise = this.normalArray(this.mean, this.stDeviation, this.numSamples);
    ...
    // Calibrated = ON / OFF  (guard against divide-by-zero)
    const eps = 1e-12;
    this.calibrated = this.onSegment.map((on, i) => on / (this.offSegment[i] + eps));

    // Plot single calibrated line (matches your matplotlib intent)
    this.plotData = [{
      type: 'scatter',
      mode: 'lines',
      name: 'calibrated',
      ...
      y: this.calibrated,
      line: { width: 2 },
      hovertemplate: 'f=%{x:.3f} MHz<br>cal=%{y:.5f}<extra></extra>',
    }];
  }
