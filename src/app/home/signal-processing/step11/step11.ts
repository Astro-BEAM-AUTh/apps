...
export class Step11 {
  ...
  private generate(): void {
    ...
    // Calibrated (difference): ON - OFF
    this.calibratedDiff = this.onSegment.map((on, i) => on - this.offSegment[i]);
    ...
    this.plotData = [{
      ...
      name: 'calibrated',
      x: this.freqs,
      y: this.calibratedDiff,
      line: { width: 2 },
      hovertemplate: 'f=%{x:.3f} MHz<br>Δ=%{y:.5f}<extra></extra>',
    }];
  }
